/**
 * Process (TODO after, in another file):
 * 1. read concatenated images
 * 2. read timestamps
 * 3. split concatenated images
 * 4. for each image: undistort then remap for getting rectified image
*/
...
int main(int argc, char **argv)
{
    if(argc != 5)
    {
        cerr << endl << "Usage: ./stereo_flexy <path_to_vocabulary> <path_to_settings> <path_to_sequence> <dir_trajectory_result>" << endl;
        return 1;
    }
    ...
    string dirTrajResults(string(argv[4]) + "/Flexy/ORB_SLAM2/" + sequenceID + "/" + sequenceID + ".txt");
